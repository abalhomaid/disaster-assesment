...
def main(args: argparse.Namespace):
    ...
    train_source_dataset, train_target_dataset, val_dataset, test_dataset, num_classes, args.class_names = \
        utils.da_get_dataset(args.data, args.root, args.source, args.target, train_transform, val_transform)
    ...
    correlation_alignment_loss = CorrelationAlignmentLoss(args.guassian).to(device)
    ...
    if args.phase == 'test':
        ...
        # base_path = f'/home/local/QCRI/abalhomaid/projects/disaster/Transfer-Learning-Library/train_jobs/{args.model_name}'
        metrics = pd.DataFrame(metrics, columns=['exp', 'accuracy', 'precision', 'recall', 'f1'])
        # print(metrics)
        # print(f'Saving to {args.log}/metrics_output.csv')
        confusion_matrix = pd.DataFrame(cm)
        metrics.to_csv(os.path.join(args.log, 'metrics_output.csv'), index=False)
        confusion_matrix.to_csv(os.path.join(args.log, 'confusion_matrix.csv'), index=False)
        return
    ...
    for epoch in range(args.epochs):
        ...
        train(train_source_iter, train_target_iter, 
              classifier, optimizer, lr_scheduler, 
              correlation_alignment_loss, args.n_domains_per_batch, epoch, args)
...
def train(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator, 
          model: utils.ImageClassifier, optimizer, lr_scheduler: CosineAnnealingLR,
    ...
    progress = ProgressMeter(
        ...
        [batch_time, data_time, losses, losses_ce, trans_losses, cls_accs],
    ...
    for i in range(args.iters_per_epoch):
        ...
        # CORAL's new loss: discrepancy_slice_wasserstein(y_t,y_s)
        ...
        # transfer_loss -> loss_penalty

        # loss = cls_loss + transfer_loss * args.trade_off ->
        # loss = loss_ce + loss_penalty * args.trade_off
        cls_loss = F.cross_entropy(y_s, labels_s)
        transfer_loss = correlation_alignment_loss(f_s, f_t)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--guassian", action='store_true', help='Set to use MMD, omit to use CORAL')
